...
int	main(int ac, char **av)
{
	...
	if (ac == 2 && !ft_strncmp(av[1], MANDELBROT, ft_strlen(MANDELBROT)) && check_demantion(WIDTH, 	HEIGHT))
		fractol_helper(&data, av);
	else if (ac == 4 && !ft_strncmp(av[1], JULIA, ft_strlen(JULIA) && check_demantion(WIDTH, HEIGHT))
	...
	else if (ac == 2 && !ft_strncmp(av[1], BURNINGSHIP, ft_strlen(BURNINGSHIP)) && check_demantion(WIDTH, HEIGHT))
		fractol_helper(&data, av);
	else
