...
void	julia_render(t_data *data, int x, int y)
{
	...
	c.x = data->x_julia;
	c.y = data->y_julia;
	z.x = (scale(x, data->x0, data->x1, WIDTH) * data->zoom) + data->shift_x;
	z.y = (scale(y, data->y0, data->y1, HEIGHT) * data->zoom) + data->shift_y;
...
void	burningship_set(t_data *data, int x, int y)
{
	...
	z.y = 0;
	i = 0;
	c.x = (scale(x, data->x0, data->x1, WIDTH) * data->zoom) + data->shift_x;
	c.y = (scale(y, data->y0, data->y1, HEIGHT) * data->zoom) + data->shift_y;
	while ((z.x * z.x + z.y * z.y) <= 4 && i < data->iterations)
	{
		tmpx = ((z.x * z.x) - (z.y * z.y)) + c.x;
